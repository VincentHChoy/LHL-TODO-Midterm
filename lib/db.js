...
const voteOnPoll = (pollId, opt1Rank, opt2Rank, opt3Rank, opt4Rank) => {
  ...
  return db
    .query(
      ...
      RETURNING *;`, [pollId, opt1Rank, opt2Rank, opt3Rank, opt4Rank]
    )
    .then(data => data.rows[0])
    .catch(err => console.error(err.stack));
};
...
const createPoll = (email, questionText) => {
  return db
    .query(
      `INSERT INTO polls (owner_email, question_text, created_at)
...
const getAllVotes = (pollId) => {
  //lowest score =
  return db
  .query(`SELECT DISTINCT SUM(opt1_rank) as opt1, SUM(opt2_rank) as opt2, SUM(opt3_rank) as opt3, SUM(opt4_rank) as opt4
  FROM votes
  JOIN options ON votes.poll_id = options.poll_id
  WHERE votes.poll_id = $1
  GROUP BY options;`, [pollId])
  .then(data => data.rows)
  .catch(err => console.error(err.stack));
};
